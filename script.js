...
function abrirModal(id) {
    ...
    // 🎯 RESETA A ESCOLHA DE TAMANHO ANTIGA AO ABRIR UM NOVO PRODUTO
    tamanhoSelectedNoModal = null; // Crie essa let no topo do arquivo se der erro de definição
...
// 🎯 GERAR OS BOTÕES DE TAMANHO DINAMICAMENTE DENTRO DO MODAL
    ...
    if (containerTamanhos) {
        ...
        listaTamanhosPadrao.forEach(tam => {
            ...
            // Lógica de clique de seleção do tamanho
            botao.onclick = () => {
                // Remove o destaque de todos os botões de tamanho do modal
                ...
                tamanhoSelectedNoModal = tam; // Salva o tamanho escolhido na memória
                
                // Libera o botão de comprar interno do modal
                if (btnAddModal) {
    ...
    // 🎯 CONFIGURAR A AÇÃO DO BOTÃO ADICIONAR DENTRO DO MODAL
    if (btnAddModal) {
        btnAddModal.onclick = () => {
            // Chama a função nova do Bloco 7 que criamos no passo anterior!
            adicionarAoCarrinho(prod.id, prod.nome, prod.preco, prod.quantidadeAtual, tamanhoSelectedNoModal);
            fecharModal(); // Fecha o modal após colocar no carrinho
        };
    }
